...
from churn_classification_dbx.common import MLflowTrackingConfig, FeatureStoreTableConfig, LabelsTableConfig
from churn_classification_dbx.model_train_pipeline import ModelTrainPipeline
from churn_classification_dbx.create_webhook import WebhookCreationConfig, WebhookCreation
...
class ModelTrainConfig:
    ...
    Attributes:
        ...
        pipeline_params (dict):
            Params to use in preprocessing pipeline. Read from model_train_config.yml
        ...
        model_params (dict):
            Dictionary of params for model. Read from model_train_config.yml
        conf (dict):
...
class ModelTrain:
    ...
    def eval_and_log_metrics(self, run, x, y):
        _df = x.copy()
        _df["y"] = y
        mlflow.evaluate(
            model=run,
    ...
    def run(self):
        ...
        Steps:
            ...
            8. Create a webhook to run a job to execute sanity checks and promote the model to Staging
        ...
        with mlflow.start_run(run_name=mlflow_tracking_cfg.run_name) as mlflow_run:
            ...
            model_url = f"runs:/{mlflow_run.info.run_uuid}/model"
            self.eval_and_log_metrics(model_url, X_test, y_test)
